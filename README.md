...
> **Status:** `v0.6.0` — Phase 0 done, Phase 1 underway. `iron-partition`
...
> AD/RFC 2307 schema validation) and `iron-kdc` (Kerberos 5 KDC: AS-REQ/
> AS-REP with pre-auth, TGS-REQ/TGS-REP, keytab I/O) are real and verified
> against a live fastetcd cluster with real `openldap-clients` and
> `krb5-workstation` tools — `iron-ldap` deployed redundantly (3 replicas +
